...
3. **CORS issues (browser clients)**
   Agents handle WebSocket upgrades automatically, but ensure your entry worker doesn't block the
   request.
...
2. **Agent crashed before state saved**
   `setState()` is durable, but if agent crashes during processing before `setState()`, changes are
   lost.
...
`setState()` automatically syncs to all connected clients via `onStateUpdate()`. If sync isn't
working:
