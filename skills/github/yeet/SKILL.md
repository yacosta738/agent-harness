...
- Prefer the configured GitHub integration for pull request creation after the branch is on the remote.
...
- Branch: semantic prefix plus short slug, for example `feat/cloudflare-skills`, `fix/sentry-path`, or `chore/github-skill-cleanup`.
- Commit: Conventional Commits, for example `feat: add Cloudflare skills integration`.
- PR title: semantic title matching the change intent, for example `feat: add Cloudflare skills integration` or `fix: correct Sentry skill script path`.
...
2. Determine the branch strategy.
   - If on `main`, `master`, or another default branch, create a semantic branch such as `feat/{description}`, `fix/{description}`, or `chore/{description}`.
