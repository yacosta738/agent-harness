...
This workflow is hybrid by design:

- Use the configured GitHub integration for PR metadata, changed files, and review context.
- Use `gh` for GitHub Actions checks and logs because the connector does not expose that workflow
  end to end.
- Summarize the root cause first, propose a focused fix plan, and implement only after explicit
  approval.

Prereq: authenticate with GitHub CLI once, then confirm with `gh auth status`. Repo and workflow
scopes are typically required for Actions inspection.
...
2. Resolve the PR.
  ...
  - When repo and PR are known, fetch PR metadata and patch context through the configured GitHub
    integration.
...
8. Recheck status and summarize residual risk.
  ...
  - Report what is still unverified, what may still be flaky, and whether any failing checks were
    external and therefore not actionable here.
...
- Do not imply that the GitHub integration can replace `gh` for Actions log retrieval.
- Treat non-GitHub Actions providers as report-only unless the user explicitly wants a separate
  investigation path.
