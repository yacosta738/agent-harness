...
This skill provides a structured workflow for managing issues, projects, and team workflows in Linear. It assumes the configured Linear integration is enabled so the available tools can read and update issues, projects, documentation, and collaboration data.
...
1. Enable the `linear` MCP entry in `editors/agents/opencode/opencode.json` for this environment or session.
...
When the user asks for heavier planning, issue triage across multiple tickets, or project orchestration, prefer routing to the `linear-pm` subagent after clarifying scope. Use this skill for direct workflow guidance and smaller, focused Linear operations.
