...
Use this skill when the user wants to work through requested changes on a GitHub pull request. Use the configured GitHub integration for PR metadata and patch context, but treat thread-aware review data as a `gh api graphql` problem because the connector comment surface is flat and does not preserve full review-thread state.
...
2. Inspect review context with thread-aware reads.
   - Use the configured GitHub integration to fetch PR metadata and patch context when the repo and PR are known.
