...
Use this when command handling depends on the active window, first responder, or AppKit menu
validation.

## Core patterns

- Start with SwiftUI `commands`, `FocusedValue`, and focused scene state.
- Use AppKit responder-chain hooks only when command routing or validation truly depends on the
  underlying responder system.
...
- Avoid scattering command logic between SwiftUI closures and AppKit selectors without a clear
  boundary.
