...
3) Pull any prior context from Notion with the available search tool → page-read tool (existing pages to update/link).
4) Draft the page with the available Notion create tool using the database schema; include summary, context, source links, and tags/owners.
5) Link from hub pages and related records; update status/owners with the available Notion update tool as the source evolves.
...
- Use the available Notion create tool with the correct data-source identifier; set properties (title, tags, owner, status, dates, relations).
