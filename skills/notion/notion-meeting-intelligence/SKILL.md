...
2) Gather context: search with the available Notion search tool, then fetch with the available Notion page-read tool (prior notes, specs, OKRs, decisions).
3) Pick the right template via `reference/template-selection-guide.md` (status, decision, planning, retro, 1:1, brainstorming).
4) Draft agenda/pre-read in Notion with the available page/database create tool, embedding source links and owner/timeboxes.
5) Enrich with Codex research (industry insights, benchmarks, risks) and update the page with the available Notion update tool as plans change.
...
- Search Notion for relevant docs, past notes, specs, and action items with the available search tool, then fetch key pages with the available page-read tool.
...
- Update the page via the available Notion update tool when details change; keep a brief changelog if multiple edits.
