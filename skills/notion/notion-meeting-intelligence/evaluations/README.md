...
- Pre-read follows structure: Overview → Background → Current Status → Context & Insights →
  Discussion Points
- Agenda follows structure: Details → Objective → Agenda Items (with times) → Decisions → Actions →
  Resources
...
1. **Test different meeting types** - Decision, status, brainstorm, 1:1, sprint planning,
   retrospective
