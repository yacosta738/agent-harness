...
Create one DO per logical unit needing coordination: chat room, game session, document, user,
tenant.
...
For hierarchical data, create separate child DOs. Parent tracks references, children handle own
state.
...
**Note:** `PRAGMA user_version` is **not supported** in Durable Objects SQLite storage. Use a
`_sql_schema_migrations` table instead:
...
`durable-utils`](https://github.com/lambrospetrou/durable-utils#sqlite-schema-migrations) — provides
a `SQLSchemaMigrations` class that tracks executed migrations both in memory and in storage. Also
see [
`@cloudflare/actors` storage utilities](https://github.com/cloudflare/actors/blob/main/packages/storage/src/sql-schema-migrations.ts) —
...
Storage operations automatically block other requests (input gates). Responses wait for writes (
output gates).
...
**Note**: Uncaught exceptions may terminate the DO instance. In-memory state is lost, but SQLite
storage persists.
