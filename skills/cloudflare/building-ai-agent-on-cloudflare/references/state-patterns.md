...
State is automatically persisted to the `cf_agents_state` SQL table. The `this.state` getter lazily
loads from storage, while `this.setState()` serializes and persists changes. State survives Durable
Object evictions.
...
The SDK includes a built-in queue for background task processing. Tasks are stored in SQLite and
...
Custom methods automatically have full agent context. Use `getCurrentAgent()` to access context from
external functions.
